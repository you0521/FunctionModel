...
public class StatusBarUtils {
    public static boolean supportTransparentStatusBar() {
        return OSUtils.isMiui()
                || OSUtils.isFlyme()
                || (OSUtils.isOppo() && Build.VERSION.SDK_INT >= OSConstant.SDKVersionCode.LOLLIPOP)
                || Build.VERSION.SDK_INT >= OSConstant.SDKVersionCode.M;
    }
    ...
    public static void transparentStatusBar(Window window) {
        if (OSUtils.isMiui() || OSUtils.isFlyme()) {
            if (Build.VERSION.SDK_INT >= OSConstant.SDKVersionCode.LOLLIPOP) {
                transparentStatusBarAbove21(window);
            } else if (Build.VERSION.SDK_INT >= OSConstant.SDKVersionCode.KITKAT) {
                window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            }
        } else if ((OSUtils.isOppo() && Build.VERSION.SDK_INT >= OSConstant.SDKVersionCode.LOLLIPOP)) {
            transparentStatusBarAbove21(window);
        } else if (Build.VERSION.SDK_INT >= OSConstant.SDKVersionCode.M) {
            transparentStatusBarAbove21(window);
        }
    }

    @TargetApi(OSConstant.SDKVersionCode.LOLLIPOP)
    ...
    private static void setStatusBarDarkMode(Window window, boolean darkMode) {
        if (Build.VERSION.SDK_INT >= OSConstant.SDKVersionCode.M) {
            if (darkMode) {
    ...
    private static void setOppoStatusBarDarkMode(Window window, boolean darkMode) {
        ...
        } else if (Build.VERSION.SDK_INT >= OSConstant.SDKVersionCode.LOLLIPOP) {
            if (darkMode) {
    ...
    public static int generateViewId() {
        if (Build.VERSION.SDK_INT >= OSConstant.SDKVersionCode.JELLY_BEAN_MR1) {
