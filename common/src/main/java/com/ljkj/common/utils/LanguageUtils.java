...
public class LanguageUtils {
    ...
    private static void applyLanguage(@NonNull final Locale locale,
                                      ...
                                      final boolean isFollowSystem) {
        if (isFollowSystem) {
            PreferencesUtils.put(KEY_LOCALE, VALUE_FOLLOW_SYSTEM);
        } else {
            String localLanguage = locale.getLanguage();
            String localCountry = locale.getCountry();
            PreferencesUtils.put(KEY_LOCALE, localLanguage + "$" + localCountry);
        }

        updateLanguage(ActivityUtils.getApp(), locale);

        Intent intent = new Intent();
        String realActivityClassName
                = TextUtils.isEmpty(activityClassName) ? getLauncherActivity() : activityClassName;
        intent.setComponent(new ComponentName(ActivityUtils.getApp(), realActivityClassName));
        intent.addFlags(
                Intent.FLAG_ACTIVITY_NEW_TASK
                        | Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_CLEAR_TASK
        );
        ActivityUtils.getApp().startActivity(intent);
    }

    static void applyLanguage(@NonNull final Activity activity) {
        final String spLocale = (String) PreferencesUtils.get(KEY_LOCALE,"");
        ...
        if (VALUE_FOLLOW_SYSTEM.equals(spLocale)) {
            ...
            updateLanguage(ActivityUtils.getApp(), sysLocale);
        ...
        updateLanguage(ActivityUtils.getApp(), settingLocale);
    ...
    private static String getLauncherActivity() {
        ...
        intent.setPackage(ActivityUtils.getApp().getPackageName());
