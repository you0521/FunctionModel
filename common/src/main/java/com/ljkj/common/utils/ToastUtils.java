...
public class ToastUtils {
    ...
    /* y 偏移 */
    private static int yOffset = (int) (64 * ActivityUtils.getApp().getResources().getDisplayMetrics().density + 0.5);
    /* Toast 的背景色 */
    ...
    public static void setBgColor(@ColorInt int backgroundColor) {
        ToastUtils.mBgColor = backgroundColor;
    }
    ...
    public static void setTextColor(@ColorInt int textColor) {
        ToastUtils.mTextColor = textColor;
    }
    ...
    public static void show(@StringRes int resId, int duration) {
        show(ActivityUtils.getApp().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(ActivityUtils.getApp().getResources().getString(resId), args), duration);
    }
    ...
    public static void show(final CharSequence text, final int duration) {
        HANDLER.post(new Runnable() {
            ...
            public void run() {
                cancel();
                mToast = Toast.makeText(ActivityUtils.getApp(), text, duration);
                TextView tvMessage = mToast.getView().findViewById(android.R.id.message);
    ...
    private static void show(final View view, final int duration) {
        HANDLER.post(new Runnable() {
            ...
            public void run() {
                cancel();
                mToast = new Toast(ActivityUtils.getApp());
                mToast.setView(view);
    ...
    private static View getView(@LayoutRes int layoutId) {
        ...
        LayoutInflater inflate = (LayoutInflater) ActivityUtils.getApp().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
